...
export default function ThemeSummary({ chapitres }: { chapitres: Chapitre[] }) {
  ...
  return (
    <nav className="sticky top-24 self-start" aria-label="Sommaire">
      <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-gris mb-4">
        Sommaire
      </p>
            ...
            <li key={c.id}>
              <button
                ...
                className={`w-full text-left pl-3 py-1.5 border-l-2 transition-all duration-200 ${
                  isActive
                    ? 'border-plume text-encre font-semibold'
                    : 'border-transparent text-secondaire font-normal'
                }`}
              >
                <span className="font-mono text-[11px] text-gris mr-2">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <span className="text-[14px]">{c.name}</span>
                <span className="font-mono text-[10px] text-gris ml-2">
