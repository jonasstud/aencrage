...
function AudioPlayer({ audioSrc, audioPeaks }: { audioSrc?: string; audioPeaks?: number[] }) {
  ...
  const [computedPeaks, setComputedPeaks] = useState<number[]>([]);
  ...
  return (
    ...
    >
      ...
      {/* Row 3 col 1: play/pause */}
      <button
        onClick={togglePlayPause}
        disabled={disabled}
        className={`flex items-center justify-center shrink-0 rounded-full border border-encre bg-white ${disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`}
